...
const API_URL = "https://script.google.com/macros/s/AKfycbzW8WO38mpPiRlePaS7Qhz9UerlggULMjbDs4X9B30dcLiX_DozvzFYxJ4RHACDrfnN/exec";
...
function loadLiveData() {
  ...
  // Menambahkan timestamp (?t=...) agar browser tidak menggunakan cache lama yang bikin "Failed to fetch"
  const secureUrl = API_URL + (API_URL.includes("?") ? "&" : "?") + "t=" + Date.now();
  ...
  fetch(secureUrl)
    ...
    .then(text => {
      try {
        ...
        throw new Error(e.message || "Data dari server tidak berformat JSON.");
      }
    })
    .catch(error => {
      console.error(error);
...
function populateFilters(){
  ...
  document.getElementById("result-category").innerHTML=optionList(cats,"Semua kategori");
  document.getElementById("all-category").innerHTML=optionList(cats,"Semua kategori");
...
function renderAffordable(){
  ...
  const items=DATA.menu.filter(m=>m.points<=selectedCustomer.points&&(!cat||m.category===cat)).sort((a,b)=>a.points-b.points);
  document.getElementById("affordable-grid").innerHTML=items.map(menuCard).join("");
...
function renderResult(customer){
  ...
  document.getElementById("result-name").textContent=customer.name;
  document.getElementById("result-avatar").textContent=(customer.name[0]||"R").toUpperCase();
  document.getElementById("result-points").textContent=formatPoints(customer.points);
  const chosen=document.getElementById("category-filter").value;
  document.getElementById("result-category").value=chosen;
...
function bindEvents(){
  ...
  document.getElementById("clear-btn").onclick=()=>{document.getElementById("result-section").classList.add("hidden");document.getElementById("customer-search").value="";document.getElementById("category-filter").value="";selectedCustomer=null;selectedReward=null;updateWhatsApp();};
  document.getElementById("category-filter").onchange=()=>{if(selectedCustomer)renderAffordable();};
  document.getElementById("result-category").onchange=renderAffordable;
